...
export default function(pi: ExtensionAPI): void {
	...
	async function stashContent(
		...
		if (!text || text.trim().length === 0) {
			ctx.ui.notify(
				"Nothing to stash after removing command",
				"warning",
			);
			return;
		}
		...
		ctx.ui.notify(
			"Stashed! Will restore to editor after next prompt",
			"info",
		);
	}
	...
	pi.registerCommand("stash", {
		description:
			"Stash current editor content (restores to editor after next prompt)",
	...
	pi.on("input", async (event, ctx) => {
		...
		if (stashMatch) {
			const textToStash = event.text.replace(
				/\s*\/stash\s*$/i,
				"",
			).trim();
	...
	pi.on("agent_end", async (_event, ctx) => {
		if (pendingRestore && ctx.hasUI) {
			...
			ctx.ui.notify(
				"Stashed content restored to editor!",
				"info",
			);
