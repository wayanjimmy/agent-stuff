...
const WebSearchParams = Type.Object({
  ...
  query: Type.Optional(
    Type.String({
      description: "Deprecated — use objective instead. Treated as objective if objective is empty.",
    }),
  ),
});
...
async function performSearch(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        const errorData = (await response.json()) as { detail?: { error?: string }; error?: string };
...
function createWebSearchTool(deps: WebSearchToolDeps) {
  return {
    ...
    async execute(
      ...
      rawParams: unknown,
      signal?: AbortSignal,
      onUpdate?: (update: { content: Array<{ type: string; text: string }>; details: SearchState }) => void,
    ) {
