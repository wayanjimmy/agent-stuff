...
export default function sourcegraphLibrarianExtension(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    label: "Librarian",
    description:
      "Cross-repo code researcher using Sourcegraph. Use when the answer likely lives in public codebases, exact locations are unknown, or you need to find implementations, patterns, or usage examples across open-source repositories. Supports broad code search, regex, symbols, and diffs. Returns evidence-first findings with verifiable links.",
    ...
    async execute(_toolCallId, params, signal, onUpdate, ctx: ExtensionContext) {
      ...
      try {
        const query =
          typeof (params as any).query === "string" ? ((params as any).query as string).trim() : "";
        if (!query) {
          return {
            content: [
              {
        ...
        if (!subModel) {
          const error =
            "No model selected. Configure credentials (e.g. /login or auth.json) and try again.";
          runs[0].status = "error";
          runs[0].error = error;
    ...
    renderCall(args, theme) {
      const query =
        typeof (args as any)?.query === "string" ? ((args as any).query as string).trim() : "";
    ...
    renderResult(result, { expanded, isPartial }, theme) {
      ...
      const backendLabel = details.backends?.join("+") ?? "sourcegraph";
      const header =
        icon +
        " " +
      ...
      const combined =
        (result.content[0]?.type === "text"
          ...
          : renderCombinedMarkdown(details.runs)
        ).trim() || "(no output)";
