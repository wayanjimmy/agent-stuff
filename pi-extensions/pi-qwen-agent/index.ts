...
import { runQwenAgent } from "./qwen-runner.ts";
...
export default function qwenAgentExtension(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal, onUpdate) {
      ...
      const result = await runQwenAgent({
        ...
        includeDirectories: Array.isArray(p.include_directories)
  ...
  pi.registerCommand("qwen", {
    ...
    async handler(args: string, ctx: ExtensionCommandContext) {
      ...
      pi.sendUserMessage(
        `Use the qwen_agent tool with approval_mode "yolo" for this task: ${task}`,
      );
    },
  });
}
